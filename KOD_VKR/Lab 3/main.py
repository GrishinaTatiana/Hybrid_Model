...
def run_comparison_only_temperature(steps=200, n_tellers=3, spawn_rate=0.7):
    ...
    print(f"{'=' * 50}")
    print(f"Средняя температура (SD):        {np.mean(temps_sd):.2f}°C")
    print(f"Средняя температура (упрощ.):    {np.mean(temps_no_sd):.2f}°C")
    print(f"Максимальная температура (SD):   {max(temps_sd):.2f}°C")
    print(f"Максимальная температура (упрощ.): {max(temps_no_sd):.2f}°C")
    print(f"Финальная температура (SD):      {temps_sd[-1]:.2f}°C")
    print(f"Финальная температура (упрощ.):  {temps_no_sd[-1]:.2f}°C")
