...
class ClientAgent(Agent):
    ...
    # Проверяет, не должен ли агент уйти из-за жары
    def check_heat_patience(self):
        temp = self.model.temperature

        # Температурный порог, начиная с которого проверяем уход
        if temp <= 28:
            return False

        # Рассчитываем вероятность ухода в зависимости от типа клиента
        if self.client_type == "HEAT_SENSITIVE":
            probability = 0.5 + (temp - 28) * 0.05
        else:  # HEAT_RESISTANT
            probability = 0.2 + (temp - 28) * 0.02

        # Ограничиваем вероятность максимумом 0.9
        probability = min(probability, 0.9)

        # Решаем, уходит ли клиент
    ...
    def step(self):
        try:
            ...
            if self.status == "ENTERING":
                ...
                # Проверка ухода из-за жары
                if self.check_heat_patience():
                    return
            ...
            # состояние 4: ушел из за жары
                ...
                # состояние 5: ушел из-за ошибки кассира
            elif self.status == "LEFT_ERROR":
                    # Проверка существования агента
                    if self.pos is None:
                        self.remove_from_model()
                        return
