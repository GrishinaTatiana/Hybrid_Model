...
class BankModel(Model):
    def __init__(self, width=20, height=10, n_tellers=2, spawn_rate=0.1):
        ...
        self.base_spawn_rate = spawn_rate  # Сохраняем базовый spawn_rate
        self.spawn_rate = spawn_rate

        # Счетчик шагов (заменяет self.schedule.steps)
        self.current_step = 0

        # ========== ДОБАВЛЯЕМ: Флаг кондиционера ==========
        self.ac_on = False
        self.AC_POWER = 500  # Мощность кондиционера в Ваттах
        self.base_insulation_factor = 5.0  # уменьшаем скорость остывания помещения
        self.modified_insulation_factor = 110.0  # Увеличенный фактор при включенном кондиционере
        ...
        self.HEAT_PER_PERSON = 80.0
        self.INSULATION_FACTOR = self.base_insulation_factor  # Начинаем с базового
        self.OUTSIDE_TEMP = 20.0  # Изменено с 15 на 20 для соответствия .mdl
        self.AIR_HEAT_CAPACITY = 50000.0  # Изменено с 50000 на 10000
        ...
        try:
            ...
            self.sd_model.set_components({
                ...
                'AC_POWER': 0  # Начинаем с выключенным кондиционером
            })
        ...
        self.datacollector = DataCollector(
            model_reporters={
                ...
                "Ушли от ошибки (всего)": lambda m: m.left_by_error,
                "Температура (SD)": lambda m: m.temperature,
                "Кондиционер включен": lambda m: m.ac_on,  # ДОБАВЛЯЕМ
                "Текущий spawn_rate": lambda m: m.spawn_rate  # ДОБАВЛЯЕМ
            }
        )

    # ДОБАВЛЯЕМ: Метод управления кондиционером
    def manage_air_conditioner(self):
        """Управление кондиционером на основе температуры"""

        if self.temperature > 26.0 and not self.ac_on:
            # Включаем кондиционер
            self.ac_on = True
            self.spawn_rate = self.base_spawn_rate * 0.3  # Уменьшаем на 70%
            self.INSULATION_FACTOR = self.modified_insulation_factor  # Увеличиваем теплоизоляцию

            # Обновляем параметры в SD модели
            if self.sd_model is not None:
                try:
                    ...
                    print(f"Ошибка обновления SD модели (включение AC): {e}")

            print(f"[Шаг {self.current_step}] ВКЛЮЧЕН кондиционер! Температура: {self.temperature:.1f}°C, "
                  f"spawn_rate: {self.spawn_rate:.2f}, InsulationFactor: {self.INSULATION_FACTOR}")

        elif self.temperature < 24.0 and self.ac_on:
            # Выключаем кондиционер
            self.ac_on = False
            self.spawn_rate = self.base_spawn_rate  # Восстанавливаем базовый spawn_rate
            self.INSULATION_FACTOR = self.base_insulation_factor  # Восстанавливаем базовую теплоизоляцию
            ...
            if self.sd_model is not None:
                try:
                    ...
                    print(f"Ошибка обновления SD модели (выключение AC): {e}")

            print(f"[Шаг {self.current_step}] ВЫКЛЮЧЕН кондиционер! Температура: {self.temperature:.1f}°C, "
                  f"spawn_rate: {self.spawn_rate:.2f}, InsulationFactor: {self.INSULATION_FACTOR}")
    ...
    def _fallback_temperature_update(self, total_people):
        """Fallback метод (если SD модель не загрузилась) с учетом кондиционера"""
        heat_gain = total_people * self.HEAT_PER_PERSON

        # Рассчитываем теплопотери с учетом кондиционера
        heat_loss = (self.temperature - self.OUTSIDE_TEMP) * self.INSULATION_FACTOR
        if self.ac_on:
            heat_loss += self.AC_POWER  # Добавляем охлаждение от кондиционера
    ...
    def step(self):
        ...
        # ШАГ 1.5: ДОБАВЛЯЕМ - Управление кондиционером после обновления температуры
        self.manage_air_conditioner()

        # ШАГ 2: Агентное моделирование - создание новых агентов (используем обновленный spawn_rate)
        ...
        # Перебираем агентов в случайном порядке (аналог RandomActivation)
        for agent in random.sample(list(self.agents), len(self.agents)):
            if agent.status != "DELETED":
                ...
                # Агент ушел из-за ошибки кассира (уникальный функционал)
