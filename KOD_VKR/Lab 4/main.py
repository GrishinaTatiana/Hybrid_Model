...
def run_simulation(steps=200, visualize=True):
    ...
    if visualize:
        ...
        ax_ac = fig.add_subplot(gs[2])  # ДОБАВЛЯЕМ: график работы кондиционера
        ...
        # ДОБАВЛЯЕМ: настройка графика кондиционера
    ...
    temperature_history = []
    ac_status_history = []  # ДОБАВЛЯЕМ
    spawn_rate_history = []  # ДОБАВЛЯЕМ
    ...
    for step in range(steps):
        ...
        ac_status_history.append(1 if model.ac_on else 0)  # ДОБАВЛЯЕМ
        spawn_rate_history.append(model.spawn_rate)  # ДОБАВЛЯЕМ
        ...
        if visualize and plt.fignum_exists(fig.number):
            ...
            line_ac.set_data(steps_history, ac_status_history)  # ДОБАВЛЯЕМ
            ...
            for text in ax_temp.texts:
                text.remove()
            for text in ax_ac.texts:  # ДОБАВЛЯЕМ
                text.remove()
