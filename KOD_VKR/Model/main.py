...
def run_simulation(steps=200, visualize=True):
    n_tellers = 3
    model = BankModel(width=20, height=n_tellers * 2, n_tellers=n_tellers, spawn_rate=0.3)
    ...
    if visualize:
        ...
        line_left, = ax_clients.plot([], [], 'r-', linewidth=1.5, label='Ушедшие из-за жары')
        line_temp, = ax_temp.plot([], [], 'r-', linewidth=1.5, label='Температура')
        ax_temp.axhline(y=28, color='orange', linestyle='--', linewidth=1, label='Порог (28°C)')
    ...
    for step in range(steps):
        ...
        # текущие значения (сколько агентов сейчас в статусе SERVED и LEFT)
